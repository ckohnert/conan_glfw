...
class glfw3Conan(ConanFile):
	...
	def system_requirements(self):
		...
		# On Mac, we only need the standard build tools, which will toss an error during
		# the CMake process when we try to build (when it fails to detect them).
	...
	def build(self):
		...
		args  = ["-DBUILD_SHARED_LIBS=" + ("ON" if self.options.shared else "OFF")]
	...
	def package_info(self):
		if self.settings.os == "Linux":
			...
			# TODO: We should be able to extract this from the pkg-config info that is built as part of glfw3
			# but for the life of me I can't see how to call pkg-config without having installed it system
			# wide...
			self.cpp_info.sharedlinkflags = [ "-framework Cocoa -framework IOKit -framework CoreFoundation -framework CoreVideo"]
			self.cpp_info.exelinkflags = [ "-framework Cocoa -framework IOKit -framework CoreFoundation -framework CoreVideo"]
